...
- The task touches a module/behavior you can't confirm by reading the
  code (not "I feel unsure" — a concrete gap: an undocumented function,
  an external contract, a data shape you'd have to guess).
- Two or more implementations are genuinely viable with different
  real tradeoffs (data model shape, public API surface, sync vs. async,
  new dependency vs. hand-rolled) — not just style preference.
...
- **Match the codebase, not a style guide.** Read the surrounding file(s)
  first — naming convention, error-handling pattern, module layout — and
  mirror them exactly, even if you'd choose differently on a blank page.
